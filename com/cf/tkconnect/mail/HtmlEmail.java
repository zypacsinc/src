...
public class HtmlEmail extends MultiPartEmail
{
    ...
    public String embed(List<Map> embeddedImages) throws EmailException
    {
    	String cid = null                        ;
    	for(Map embeddedImage :embeddedImages){
    		cid = embed(embeddedImage);
    	}
    	return cid;	
    }
    public String embed(Map embeddedImage) throws EmailException
    {
        // verify that the URL is valid
	   String filename = (String) embeddedImage.get("file_name");
	   String filepathwithname = (String) embeddedImage.get("file_path")+"/"+filename;
 
        try
        {
            java.io.File file = new java.io.File(filepathwithname);
            if(!file.exists())  throw new EmailException("File Name:"+filepathwithname+" does not exists.");
        	
        }
        catch (Exception e)
        {
            throw new EmailException("Invalid File Name:"+filepathwithname);
        }
        ...
        try
        {
            mbp.setDataHandler(new DataHandler(new FileDataSource(filepathwithname)));
